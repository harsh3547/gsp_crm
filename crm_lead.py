...
class crm_lead(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('stage_id') and len(vals)==1 and not context.get('cancel_sale_order',False):
            crm=self.pool.get('crm.lead').browse(cr,uid,ids[0])
            ir_model_data = self.pool.get('ir.model.data')
            crm_case_closed_id = ir_model_data.get_object_reference(cr, uid, 'crm', 'stage_lead6')[1]
            crm_case_cancel_id = ir_model_data.get_object_reference(cr, uid, 'crm', 'stage_lead7')[1]
            if (crm.ref and crm.ref._name=='sale.order') and crm.stage_id.id==crm_case_closed_id:
                raise osv.except_osv(_('Warning!'), _('You cannot change the state of opportunity after it has been won.'))
            if (crm.ref and crm.ref._name=='sale.order') and crm.stage_id.id==crm_case_cancel_id:
                raise osv.except_osv(_('Warning!'), _('You cannot change the state of opportunity after it has been lost.'))
            if (crm.ref and (crm.ref._name=='sale.order' and crm.ref.state in ['draft','state'])) and vals['stage_id']==crm_case_closed_id:
    ...
    def case_mark_won(self, cr, uid, ids, context=None):
        ...
        ref = self.browse(cr,uid,ids[0]).ref
        if context.get('via_button',True) and (ref and ref.state in ['sent','draft']):
            raise osv.except_osv(_('Warning!'), _('Please confirm the related quotation to mark the opportunity as won.'))        
